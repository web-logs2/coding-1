...
public class Fat {
	...
	public Fat(byte[] fatData) {
		this.fatData = fatData;
	}


	/**
	 * 0000h	Free cluster - 对应的 cluster 是空的，没有文件占用 0001h - 0002h	Not allowed - 不允许存在的值 0003h - FFEFh ，3-65519	Number of the next cluster - 对应 cluster
	 * 只是一个文件的部分内容，后续内容在下一个 cluster，这个值就是下一个 cluster 的编码 FFF7h	One or more bad sectors in cluster - 对应 cluster 中有损坏扇区，不能使用 FFF8h	End-of-file - 对应
	 * cluster 是文件的结尾，没有后续数据了
	 */
